<template>
    <div class="xl:w-[500px] border-b border-gray-400 py-2">
        <div class="flex justify-between py-4 gap-2 ">
            <!-- Item Image  -->
            <div class="">
                <NuxtImg src="v1695223477/2O0A2680_t8cqwg.webp"
                         :alt="name"
                         provider="cloudinary"
                         loading="lazy"
                         sizes="sm:100px md:50vw lg:400px"
                         format="webp"
                         class="object-cover w-20 h-[70px] rounded-sm shadow-md shadow-black" />
            </div>
            <!-- Item Name and Details  -->
            <div class="flex flex-col gap-2 basis-3/5">
                <!-- Item Name  -->
                <p class="font-semibold text-[22px] tracking-wide md:text-[25px] md:font-bold">
                    {{ name }}
                </p>
                <!-- Item Details  -->
                <p class="font-jost text-[#4a4a4b] text-[14px] md:text-[16px]">
                    {{ ingredients }}
                </p>
            </div>
            <!-- Item Price  -->
            <div class="text-[#272727] place-self-start font-jost  md:text-[16px]">

                {{ `Ksh${price}` }}

            </div>
        </div>

    </div>
</template>

<script setup>
defineProps({
    name: {
        type: String,
        default: 'Item Name'
    },
    ingredients: {
        type: String,
        default: 'Item Ingredients'
    },
    price: {

        default: '$0.00'
    },
    image: {
        type: String,
        default: 'v1694095332/2O0A2492_xhfbyj.webp'
    }
})


</script>

<style lang="scss" scoped></style>